"use strict";
// Transcrypt'ed from Python, 2018-05-14 20:33:19
function fc_solve_find_index_s2ints () {
		...
		're.translate', {
			__all__: {
				...
				__init__: function (__all__) {
					...
					var VERBOSE = false;
					var MAX_SHIFTREDUCE_LOOPS = 100000;
	...
	(function () {
		...
		__nest__ (re, '', __init__ (__world__.re));
		var Card = __class__ ('Card', [object], {
			__module__: __name__,
			ACE: 1,
			KING: 13,
			get __init__ () {return __get__ (this, function (self, id, rank, suit, print_ts) {
				var __left0__ = tuple ([id, rank, suit, print_ts]);
				self.id = __left0__ [0];
				self.rank = __left0__ [1];
				self.suit = __left0__ [2];
				self.print_ts = __left0__ [3];
				var __left0__ = tuple ([false, false]);
				self.empty = __left0__ [0];
				self.flipped = __left0__ [1];
			});},
			get rank_s () {return __get__ (this, function (self) {
				var ret = '0A23456789TJQK' [self.rank];
				if (!(self.print_ts) && ret == 'T') {
					var ret = '10';
				}
				return ret;
			});},
			get suit_s () {return __get__ (this, function (self) {
				return 'CSHD' [self.suit];
			});},
			get to_s () {return __get__ (this, function (self) {
				if (self.empty) {
					return '-';
				}
				var ret = self.rank_s () + self.suit_s ();
				if (self.flipped) {
					var ret = ('<' + ret) + '>';
				}
				return ret;
			});},
			get found_s () {return __get__ (this, function (self) {
				return (self.suit_s () + '-') + self.rank_s ();
			});},
			get is_ace () {return __get__ (this, function (self) {
				return self.rank == self.ACE;
			});},
			get is_king () {return __get__ (this, function (self) {
				return self.rank == self.KING;
			});},
			get flip () {return __get__ (this, function (self, flipped) {
				if (typeof flipped == 'undefined' || (flipped != null && flipped .hasOwnProperty ("__kwargtrans__"))) {;
					var flipped = true;
				};
				var ret = Card (self.id, self.rank, self.suit, self.print_ts);
				ret.flipped = flipped;
				return ret;
			});}
		});
		var createCards = function (num_decks, print_ts) {
			var ret = list ([]);
			for (var _ = 0; _ < num_decks; _++) {
				var id = 0;
				for (var s = 0; s < 4; s++) {
					for (var r = 0; r < 13; r++) {
		...
		var ms_rearrange = function (cards) {
			if (len (cards) != 52) {
				return cards;
			}
			var c = list ([]);
			for (var i = 0; i < 13; i++) {
				var __iterable0__ = tuple ([0, 39, 26, 13]);
				for (var __index0__ = 0; __index0__ < len (__iterable0__); __index0__++) {
		...
		var find_index__board_string_to_ints = function (content) {
			var rank_s = 'A23456789TJQK';
			var rank_re = ('[' + rank_s) + ']';
			var suit_s = 'CSHD';
			var suit_re = ('[' + suit_s) + ']';
			var card_re = rank_re + suit_re;
			var card_re_paren = ('(' + card_re) + ')';
			var string_repeat_for_transcrypt = function (s, n) {
				if (n == 0) {
					return '';
				}
			...
			var make_line = function (n) {
				return ((':?[ \\t]*' + card_re_paren) + string_repeat_for_transcrypt ('[ \\t]+' + card_re_paren, n - 1)) + '[ \\t]*\\n';
			};
			var complete_re = (('^' + string_repeat_for_transcrypt (make_line (7), 4)) + string_repeat_for_transcrypt (make_line (6), 4)) + '\\s*$';
			var m = re.match (complete_re, content);
			if (!(m)) {
				var __except0__ = ValueError ('Could not match.');
				__except0__.__cause__ = null;
				throw __except0__;
			}
			var cards = (function () {
				var __accu0__ = [];
				var __iterable0__ = ms_rearrange (createCards (1, true));
				for (var __index0__ = 0; __index0__ < len (__iterable0__); __index0__++) {
					var x = __iterable0__ [__index0__];
					__accu0__.append (x.to_s ());
				}
				return __accu0__;
			}) ();
			var ints = list ([]);
			var __left0__ = 4 * 13 - 1;
			var limit = __left0__;
			var n = __left0__;
			for (var i = 0; i < limit; i++) {
				var col = Math.floor (i / 8);
				var row = __mod__ (i, 8);
				var s = m.group ((1 + col) + (row >= 4 ? 4 * 7 + (row - 4) * 6 : row * 7));
				var idx = (function () {
					var __accu0__ = [];
					for (var j = 0; j < n + 1; j++) {
						if (cards [j] == s) {
				...
				if (len (idx) != 1) {
					var __except0__ = ValueError ('Foo');
					__except0__.__cause__ = null;
					throw __except0__;
				}
				var j = idx [0];
		...
		var factory = function () {
			return dict ({'find_index__board_string_to_ints': find_index__board_string_to_ints});
		};
		...
		__pragma__ ('<all>')
			__all__.Card = Card;
			__all__.__name__ = __name__;
			__all__.createCards = createCards;
			__all__.f = f;
			__all__.factory = factory;
			__all__.find_index__board_string_to_ints = find_index__board_string_to_ints;
			__all__.ms_rearrange = ms_rearrange;
		__pragma__ ('</all>')
	}) ();
    return __all__;
}
fc_solve_find_index_s2ints ();

//# sourceMappingURL=extra/sourcemap/fc_solve_find_index_s2ints.js.map
